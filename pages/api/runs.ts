...
import { listRuns, loadRun, saveRun } from '../../lib/runStore'
...
export function recordRun(state?: unknown) {
  if (state && typeof state === 'object' && 'runId' in (state as Record<string, unknown>)) {
    try { saveRun(state as any) } catch { /* ignore */ }
  }
  return { ok: true }
}
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    if (exportFmt === 'json') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename="meal-planner-runs.json"')
